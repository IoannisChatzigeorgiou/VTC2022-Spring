function sim_intercept_RLC_with_SD_for_fig2(K, L, e_dest, e_eaves)
% --------------------------------------------------------------------
...
% (RLC) to encode K source packets. The destination uses RLC decoding to 
% recover the K source packets. The eavesdropper combines RLC decoding with
% Syndrome Decoding (SD) in an attempt to repair erroneously received 
% packets and increase the probability of recovering the K source packets.
...
%
% Syntax:
% > sim_intercept_RLC_with_SD_for_fig2(K, L, e_dest, e_eaves)
%
% --- Input arguments ---
...
for idx = 1:length(e_dest)
    ...
    for experiment = 1:experiments
        ...
        while (get_rank_dest < K) && (get_rank_eaves < K)
            ...
            generator_matrix_dest = generator_matrix(sum(error_matrix_dest, 2)<1, :);
            
            % Check the rank of the generator matrix at the destination
            [get_rank_dest, ~] = func_bin_m2ref(generator_matrix_dest);
            ...
            if (get_rank_eaves == K)
                ...
                if N>K
                    ...
                    % Use syndrome decoding to estimate the error matrix
...
filename = ['data_RLC_SD_K',num2str(K),'_L',num2str(L),'_',str_e_eaves,'.mat'];
